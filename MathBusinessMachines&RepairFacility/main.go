...
func main() {
	...
	// Setting up machine failures and repairs
	repairRequest := make(chan RepairNeed)
	doneRepairs = make(chan RepairMach)
	repairTodos = make(chan RepairMach)
	showBrokenMachines := make(chan bool)
	go repairFacility(repairRequest, doneRepairs, repairTodos, showBrokenMachines)

	for i := 0; i < RepairWorkNum; i++ {
		go repairman(i, repairRequest, doneRepairs)
	}
	...
	for i := 0; i < WorkNum; i++ {
		...
		go worker(i, workerTodos, workerProducts, displayWorkerStats[i], patient, bossTasks)
	}
	...
	if Silent {
		for {
			...
			fmt.Println("broken - Prints machines that are broken.")
